...
export interface ProjectsListPageProps {
  typeProjet: 'CSR' | 'Cahier de charges';
  title: string;
  subtitle: string;
}

export default function ProjectsListPage({ typeProjet, title, subtitle }: ProjectsListPageProps) {
  ...
  const sectors = useMemo(() => getProjectSectors(scopedProjects), [scopedProjects]);
  const statuses = useMemo(() => getProjectStatuses(scopedProjects), [scopedProjects]);
  ...
  const filteredCount = useMemo(() => {
    ...
    return scopedProjects.filter((p) => {
      ...
      return (
        ...
        (!statut || p.statut === statut || (statut === 'Terminé' && p.statut === 'Closed'))
      );
    }).length;
  }, [scopedProjects, search, secteur, statut]);

  const openCreate = () => {
    setNewProject({ ...createEmptyProject(projects), typeProjet });
    setCreateOpen(true);
  };
  ...
  const handleCreateSave = async (project: ProjectRecord) => {
    const res = await fetch('/api/projects', {
      ...
      body: JSON.stringify({ ...project, typeProjet }),
    });
    if (!res.ok) {
            ...
            <div className="projects-header-actions check-docs-header-actions">
              <div className="panel-toolbar docs-filter-bar-compact projects-header-filters">
                ...
                <select className="filter-select" value={statut} onChange={(e) => setStatut(e.target.value)}>
                  ...
                  {statuses.map((s) => (
                    <option key={s} value={s}>{formatProjectStatus(s)}</option>
                  ))}
                </select>
