...
import { deleteVehicule, updateVehicule } from '@/lib/charroi-store';
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  try {
    ...
    const item = await updateVehicule(id.trim(), body);
    if (!item) return NextResponse.json({ error: 'Véhicule introuvable' }, { status: 404 });
...
export async function DELETE(_request: Request, { params }: Params) {
  ...
  try {
    const { id } = await params;
    if (!id?.trim()) return NextResponse.json({ error: 'ID requis' }, { status: 400 });
    const ok = await deleteVehicule(id.trim());
    if (!ok) return NextResponse.json({ error: 'Véhicule introuvable' }, { status: 404 });
