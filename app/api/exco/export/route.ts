...
import {
  buildExcoPptxBuffer,
  buildExcoPptxFilename,
} from '@/lib/exco-pptx-export';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const buffer = await buildExcoPptxBuffer(report);
    const filename = buildExcoPptxFilename(year, month);

    await auditSimpleAction({
      module: 'exco',
      action: 'export',
      summary: `Export PowerPoint EXCO ${filename}`,
    });
